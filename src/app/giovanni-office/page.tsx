...
const BOSS_MAP: number[][] = Array(MAP_H).fill(0).map((_, y) => 
  Array(MAP_W).fill(0).map((_, x) => {
    // Limits
    if (y === MAP_H - 1 && x >= 8 && x <= 11) return ROCKET_FLOOR;
    if (x === 0 || x === MAP_W - 1 || y === 0 || y === MAP_H - 1) return BOSS_DESK;
    // Desk
...
export default function GiovanniOffice() {
        ...
        return (
            <GBAShell>
                <div className="gba-screen max-w-2xl w-full p-8 text-center bg-gray-900 border-red-800 h-full flex flex-col justify-center items-center">
                    ...
                    <p className="opacity-70 text-xs mb-8">Niveau de clairance insuffisant pour le Sous-Sol -42.</p>
    ...
    const handleInteract = (tile: number, x: number, y: number, npcId?: string) => {
        ...
        if (npcId === "boss") setDialog("Giovanni : Le Projet M approche de sa phase finale. Je vois que tu as trouvé mon repaire... Déguerpis !");
        else if (tile === BOSS_DESK) setDialog("Terminal : [TENTATIVE DE CRACKING...] Code '7382-4B9F' accepté. 'Le clone 150 est trop instable... Ne l'approchez pas sans la Masterball.'");
    };
