...
const GLITCH_MAP: number[][] = Array(MAP_H).fill(0).map((_, y) =>
    Array(MAP_W).fill(0).map((_, x) => {
        // Highly disordered map
        if (x === 0 || x === MAP_W - 1 || y === 0 || y === MAP_H - 1) return GLITCH_WALL;
...
export default function GlitchCity() {
    ...
    const handleInteract = (tile: number, x: number, y: number, npcId?: string) => {
        ...
        if (npcId) {
            playGlitchSound();
            setDialog("§%* ERROR : µ@& DATA CORRUPTED 0x9F4C... Item quantity x128. SILPH SCOPE UNLOCKED. CODE (PART 2): 4B9F...");
    ...
    return (
        <GBAShell>
            <section className="relative tile-bg pixel-grid bg-white h-full" style={{ cursor: "none" }}>
                ...
                <div className="relative isolate h-full">
                    {/* Heavy noise overlay */}
                    <div className="absolute inset-0 pointer-events-none z-50 mix-blend-overlay opacity-50 bg-repeat"
