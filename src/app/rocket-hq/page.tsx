...
const ROCKET_MAP: number[][] = Array(MAP_H).fill(0).map((_, y) =>
    Array(MAP_W).fill(0).map((_, x) => {
        if (y === MAP_H - 1 && x >= 8 && x <= 11) return ROCKET_FLOOR; // Doorway
        if (x === 0 || x === MAP_W - 1 || y === 0 || y === MAP_H - 1) return ROCKET_CRATE;
        if ((x === 4 || x === 10 || x === 16) && (y === 3 || y === 8)) return ROCKET_CRATE;
        return ROCKET_FLOOR;
    })
);
...
export default function RocketHQ() {
        ...
        return (
            <GBAShell>
                <div className="gba-screen max-w-2xl w-full p-8 text-center bg-gray-900 border-gray-800 h-full flex flex-col justify-center items-center">
                    ...
                    <p className="opacity-70 text-xs mb-8">Cet entrepôt est propriété privée. Circulez.</p>
    ...
    const handleInteract = (tile: number, x: number, y: number, npcId?: string) => {
        ...
        if (npcId === "grunt1") setDialog("Sbire : Pff, j'ai paumé ma partie du code pour le labo... C'était 4B9F ou l'inverse ?");
        else if (npcId === "grunt2") setDialog("Sbire : Le projet M de Giovanni au S-S 42 avance... J'espère que cette chose est bien enfermée.");
        else if (tile === ROCKET_CRATE) setDialog("Caisse : Rien d'utile, seulement des vieux uniformes de la Team Rocket.");
    };
